...
1. Eclipse
2. the commandline (via mvn)
3. from our continuous integration server, Jenkins
4. from a web browser (via deployment of the MNWebTester .war)
   - the default configuration for web tests only runs a subset 
     of tests ("MNodeTier" tests), but theoretically can run others.
5. running the WebTester locally using an embedded Jetty server.
...
but doesn't in practice for reasons unknown.  For example the following may NOT work:
...
> **Note**:
> * for v1.x d1_integration checkouts, tests are organized by Tier;
> * for v2 the tests are organized by API names, for greater flexibility.
...
> **Tip**: You can find a full listing of available categories (MNCore*, MNRead* etc) [here for
v1](https://repository.dataone.org/software/cicore/tags/API-DOCUMENTATION-v1.1.0/MethodCrossReference.xls),
and [here for v2](https://repository.dataone.org/documents/Projects/cicore/architecture/api-documentation/MethodCrossReference.xls)
...
> ⚠️ currently broken Jan 2025

To deploy a new MNWebTester, do the following:
1. commit changes to git
2. perform 'git pull'              to bring the revision number up-to-date on you local copy
...
4. clone this project, and checkout either the development branch (`develop`) or a released branch
   (tag)
   For example:
   ```shell
   git clone git@github.com:DataONEorg/dataone-integration.git   
   git co develop
   # or
   git co 2.0.0
   ```
