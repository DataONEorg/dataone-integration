...
public class MNSystemMetadataMutabilityImplementations extends ContextAwareTestCaseDataone {
    ...
    public void testRegisterSystemMetadata_dateModified() {
        ...
        final Identifier pid = D1TypeBuilder.buildIdentifier("testRegisterSystemMetadata_dateModified_" 
                + ExampleUtilities.generateIdentifier());
        try {
                ...
                protected SystemMetadata attempt() throws TryAgainException, Exception {
                    try {
                        ...
                        if (replicaList.size() == 0) {
                            TryAgainException f = new TryAgainException();
                            f.initCause(new NotFound("404", "CN sysmeta contained an empty replica list! " + cnV2.getLatestRequestUrl()));
                            throw f;
                        }
                        ...
                        if (v2ReplicaNode == null) {
                            TryAgainException f = new TryAgainException();
                            f.initCause(new NotFound("404", "CN sysmeta contained no replica for a v2 MN! " + cnV2.getLatestRequestUrl()));
                            throw f;
                        }
                        return sysmeta;
                    } catch (NotFound | ServiceFailure e) {
