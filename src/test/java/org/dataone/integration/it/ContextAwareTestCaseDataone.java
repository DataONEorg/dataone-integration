...
//				sysMeta.getSubmitter().setValue(ownerX500);
	...
	{
		return procureTestObject(mNode,null,permissions);
	}
	 ...
	 * @throws - IndexOutOfBoundsException  - when can't procure an object Identifier
	 */
	protected Identifier procureTestObject(MNode mNode, Subject subjectFilter, Permission[] permissions) 
	{
		Identifier id = null;
		...
		try {
			...
			if (id == null) {
				...
				id = createTestObject(mNode, null, permissions, subjectFilter);
			}
		} 
	 ...
	 * @throws NotFound
	 */
	protected Identifier createPublicTestObject(MNode mNode, String idSuffix) 
	...
	{ 
		...
		return createTestObject(mNode, idSuffix, new Permission[] {Permission.READ}, subject);
	}
	 ...
	 * @throws NotFound 
	 */
	protected Identifier createTestObject(MNode mNode, String idSuffix, Permission[] permissions, Subject subject) 
	throws InvalidToken, ServiceFailure, NotAuthorized, IdentifierNotUnique, 
	UnsupportedType, InsufficientResources, InvalidSystemMetadata, NotImplemented, 
	InvalidRequest, UnsupportedEncodingException, NotFound 
	{
		...
		if (idSuffix != null) {
			pid.setValue("MNodeTierTests." + ExampleUtilities.generateIdentifier() +
					"." + idSuffix);
		} else {
			pid.setValue("MNodeTierTests." + ExampleUtilities.generateIdentifier());
		}
